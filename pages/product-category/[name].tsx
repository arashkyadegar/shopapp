...
export async function getServerSideProps(context: any) {
  ...
  const res = await fetch(`${baseURL}/api/wbcategories/${name}`);
  const repo = await res.json();
  const categories = JSON.stringify(repo);
  return { props: { name, categories } };
}

export default function Category({ name, categories }: any) {
  const category = JSON.parse(categories)[0];
  let totalSortQuesry = "";
  const downDesc = { __html: category.downDesc };
  const upDesc = { __html: category.upDesc };
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row p-10  gap-4">
        <div className="w-full flex flex-col gap-2">
          <Image
            key={category.name}
            src={getDefaultImageAvator(category.image)}
            alt={category.name}
            width={500}
            height={500}
          />
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          {category.keywords && (
            <div className="flex flex-row gap-2 text-xs ">
              ...
              <a>دسته :</a>
              {category.keywords.map((item: any) => (
      ...
      <div className="flex flex-col p-6 ">
        ...
        <div className="flex flex-col mx-10 ">
          ...
          <div dangerouslySetInnerHTML={upDesc} className="leading-relaxed" />
        </div>
        <div className="flex flex-col p-10 my-4  gap-4">
        ...
        <div className="flex flex-col mx-10  mb-10">
          <div className="border-t border-gray-200 mb-4 pt-2">
            <div dangerouslySetInnerHTML={downDesc} className="leading-relaxed" />
          </div>
        </div>
        {/* {item.extras && (
          <div className="flex flex-col mx-10  mb-10" >
